...
# Add baseline model backbone.
class AlexNet3D(nn.Module):
    # def __init__(self, num_classes=3, dropout_prob=0.5):
    def __init__(self, in_channel=1,feat_dim=1024,expansion = 4, type_name='conv3x3x3', norm_type = 'Instance'):
        super(AlexNet3D, self).__init__()
        
        # Feature extraction (matches original AlexNet structure but in 3D)
        self.features = nn.Sequential(
            # Conv1 (large kernel as in original AlexNet)
            # nn.Conv3d(in_channel, 64, kernel_size=5, stride=2, padding=2),  # Modified first layer
            nn.Conv3d(in_channel, 64, kernel_size=11, stride=4, padding=2),
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(p=0.5),
            nn.Linear(2048, 4096),  # Adjusted for 3D
            nn.ReLU(inplace=True),
